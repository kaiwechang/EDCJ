...
static int refAdjSize = 0, tarAdjSize = 0;
static bool isdebug = 0, isdebugadj = 0, isdebugall = 0;
static vector<Marker> refMarkers, tarMarkers;
static vector<GRBVar> markerVars; // ref--tar--capRef--capTar
static vector<GRBVar> homoVars;
static vector<GRBVar> cycleVars;
static vector<GRBVar> nodeVars; // ref--capRef--tar--capTar
static vector<GRBVar> adjVars;
static vector<Node> nodeVecs;	// ref--capRef--tar--capTar
static vector<Edge> homoEdges; // edge (,markeri,markerj,)
static vector<Edge> adjEdges;	// edge (,nodei,nodej,)
static vector<int> refFamSize, tarFamSize;
static vector<int> refTelomeres, tarTelomeres;
static vector<string> refContig, tarContig;
static set<int> familySet;
static map<pair<int, int>, bool> refPotentialAdj, tarPotentialAdj; //is there a contig with marker pair int,int
static map<pair<string, string>, bool> refAdj, tarAdj;				//is contig pair string,string available
static GRBLinExpr teloWithCapSum = 0;
static GRBLinExpr refRemoveMarkerNum = 0;
static int refCapSize = 0, tarCapSize = 0, rc = 0, tc = 0;
static int refNodeSize;
static int refMarkerSize = 0, tarMarkerSize = 0; //without capping gene
static int refMarkerNum = 0, tarContigNum = 0, refContigNum = 0;
static int refRealAdjNumc2c = 0, tarRealAdjNumc2c = 0;
static int refPotAdjNumt2c = 0, refPotAdjNumt2t = 0, refPotAdjNumc2c = 0, tarPotAdjNumt2c = 0, tarPotAdjNumt2t = 0, tarPotAdjNumc2c = 0;
static int jabs(int n)
{
	if (n > 0)
		return n;
	return -n;
}
static void getInput(string refFile, string tarFile)
{
	ifstream fin;
	fin.open(refFile);
	int id, family, tmp;
...
static void addDuplicatedVar(GRBModel &model)
{
...
static void addMemberConstraint(GRBModel &model)
{
...
static void addHomoVar(GRBModel &model)
{
	string prevContig = "NotAContig";
...
static void addHomoConstraint(GRBModel &model)
{
...
static void addNodeVar(GRBModel &model)
{
	int nodeid = 1;
	bool is_head;
...
static void addAdjVar(GRBModel &model)
{
	//ref
...
static void addAdjConstraint(GRBModel &model)
{
...
static void addCycleConstraint(GRBModel &model)
{
...
static void addInternalConstraint(GRBModel &model)
{
	for (int i = 0; i < refMarkerSize; i++)
	{
		if (isdebugall)
...
static void addCycleVarAndConstraint(GRBModel &model)
{
...
static void findMax(GRBModel &model)
{
...
static void showVars(bool debug)
{
	if (!debug)
		return;
...
static void showDebugInfo(bool isdebug)
{
	if (!isdebugall)
		return;
	cout << endl
...
static void graphDebug(bool debug)
{
	if (!debug)
		return;
	printf("\n===== ===== ===== graphDebug ===== ===== =====\n\n");
...
int ilp_cap(string refPath, string tarPath, string outDir, Mode mode, int gap, int procs, int timelimit)
{
	out_dir = outDir;
	try
	{
